...
export default function decorate(block) {
  ...
  if (tmpName === 'training') {
    ...
    const targetEl = document.querySelector('.elouqa-form-container');
  ...
  block
    .querySelector('h1')
    .classList.add(
      ...'lg:w-2/3 my-5 text-black-0 !text-3xl lg:!text-4xl font-semibold !tracking-wider break-words'.split(' '),
    );
  ...
  if (description) description.classList.add(...'lg:w-2/3 text-lg tracking-[0.3px] leading-7'.split(' '));
  if (block && block.classList.contains('title-image')) {
    const template = getMetadata('template');
    block.parentElement.parentElement.classList.add('relative');
    block
      .querySelector('h1')
      .parentElement.parentElement.classList.add(...'xl:min-h-[400px] w-full lg:w-1/2 lg:py-6 lg:pr-6'.split(' '));
    applyClasses(
      block,
      'text-center-align image-full-width font-sans text-base flex flex-col gap-y-6 pb-4 px-6 lg:pr-0 justify-center lg:flex-row xl:container xl:max-w-7xl mx-auto items-center',
    );
    if (getContentType() && template === 'stories') {
      block.firstElementChild?.firstElementChild?.prepend(
        div({ class: 'font-[rockwell] text-sm leading-4 text-[#435656] capitalize mb-2' }, getContentType()),
      );
    }
    const image = block.querySelector('picture');
    const source = block.querySelector('picture source');
    // increased viewport size from a min value of 600 to 1920 for displaying large images
    ...
    if (image) {
      ...
      applyClasses(
        titleImage,
        'w-full h-48 md:h-[400px] object-contain relative xl:w-1/2 xl:absolute xl:right-0 xl:top-0',
      );
    } else {
